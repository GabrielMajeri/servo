...
use crate::dom::bindings::reflector::{
    self, reflect_dom_object, DomObject, DomObjectWrap, Reflector,
};
...
#[dom_struct]
pub struct IterableIterator<T: DomObject + DomObjectWrap + JSTraceable + Iterable> {
    reflector: Reflector,
    iterable: Dom<T>,
    type_: IteratorType,
    index: Cell<u32>,
...
impl<T: DomObject + DomObjectWrap + JSTraceable + Iterable> IterableIterator<T> {
...
impl<T: DomObject + DomObjectWrap + JSTraceable + Iterable> DomObjectWrap for IterableIterator<T> {
    const WRAP: unsafe fn(JSContext, &GlobalScope, Box<Self>) -> Root<Dom<Self>> =
        reflector::invalid_wrap_function_do_not_use;
}
